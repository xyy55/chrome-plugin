$("body > div:contains('pan.baidu.com')").each(function () {
  $(this).find("a").each(function () {
    let e = $(this).html();
    if (- 1 === e.indexOf("<script")) {
      $(this).html(e.replace(/https?:\/\/pan\.baidu\.com/gi, "https:\/\/####BDY####"));
      let h = $(this).attr("href");
      if (h != null && h.match(/http:\/\/pan\.baidu\.com/gi) != null) {
        $(this).attr("href", h.replace(/http:\/\/pan\.baidu\.com/gi, "https:\/\/####BDY####"))
      }
    }
  });
  ...
  findAndReplaceDOMText($(this)[0], {
    find: /http:\/\/pan\.baidu\.com\/s\/1[a-zA-Z0-9_\-]{5,22}/gi,
    replace: function (e, t) {
      let n = document.createElement("a"),
        r = document.createTextNode(t.toString().replace(/:\/\/pan\.baidu\.com/gi, "s:\/\/####BDY####"));
      return n.href = t,
        n.target = "_blank",
        n.appendChild(r),
        n
  ...
  $(this).find("a").each(function () {
    ...
    if (- 1 === e.indexOf("<script")) {
      $(this).html(e.replace(/####BDY####/g, "pan.baidu.com"));
      let h = $(this).attr("href");
      if (h != null && h.match(/####BDY####/g) != null) {
        $(this).attr("href", h.replace(/####BDY####/g, "pan.baidu.com"))
      }
    }
  })
})
...
$("a").each(function () {
  ...
  if (e != null) {
    let url = e.match(/https?:\/\/pan.baidu.com\/s\/1[a-zA-Z0-9_\-]{5,22}/gi)
    if (url != null && urls.indexOf(url[0]) === -1) {
      urls.push(url[0])
    }
  }
})
...
chrome.runtime.sendMessage(api_urls, function (response) {
  for (let i = 0; i < response.length; i++) {
    if (response[i].state == "VALID") {
      $("a").each(function () {
        let valid_link = $(this).html().match(new RegExp(urls[i], 'gi'))
        if (valid_link != null) {
